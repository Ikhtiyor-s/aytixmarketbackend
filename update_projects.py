...
login_response = requests.post(
    ...
    data={"username": "+998900000000", "password": "admin123"}
)
...
for project in projects_data:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"[OK] Project {project_id} updated: {result.get('name_uz', 'Unknown')}")
        else:
            print(f"[FAIL] Project {project_id} failed: {response.status_code} - {response.text[:100]}")
